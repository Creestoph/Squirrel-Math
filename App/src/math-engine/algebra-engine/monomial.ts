...
export class Monomial extends Product {
    ...
    static asMonomial(e: Expression, variables?: Variable[]): Monomial {
        ...
        const coefficient = coefficientFactors.length == 1 ? coefficientFactors[0] : Product.of(...coefficientFactors);
    ...
    cleaned(): Monomial {
        ...
        for (let i = 0; i < factors.length - 1; i++) {
            if (factors[i].base.identical(factors[i + 1].base)) {
                factors[i] = new Power(
                    factors[i].base,
                    (factors[i].exponent as Integer).add(factors[i + 1].exponent as Integer),
                );
