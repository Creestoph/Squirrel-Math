...
export class Polynomial extends Sum {
        ...
        if (
            ...
            (!variables ||
                (e.variables.length == variables.length &&
                    e.variables.every((v) => variables.some((w) => v.identical(w))))) &&
            e.order == order
        ) {
            return e;
        }
        return Polynomial.fromMonomials(
            standardForm(e).addends.map((p) => Monomial.asMonomial(p, variables) as Monomial),
            order,
        ).cleaned();
    }
    ...
    cleaned(): Polynomial {
        ...
        monos = monos
            .map((m) => new Monomial(simplify(m.coefficient), [...m.variables], [...m.exponentVector]))
    ...
    add(addend: Monomial | Polynomial): Polynomial {
        if (addend instanceof Monomial) {
            return Polynomial.fromOrderedMonomials(
                insertByBisection([...this.monomials], addend, this.order.descending),
                this.order,
            );
        }
        ...
        if (addend.order != this.order) {
            if (addend.monomials.length < this.monomials.length) {
                ...
                ((monos2 = [...addend.monomials].sort(this.order.descending)), (dominatingOrder = this.order));
            } else {
                monos1 = [...addend.monomials];
                ((monos2 = [...this.monomials].sort(addend.order.descending)), (dominatingOrder = addend.order));
            }
        } else {
            monos1 = [...this.monomials];
        ...
        monos2.forEach((m) => {
            ...
            if (monos1[lastInserted] && monos1[lastInserted].body().identical(m.body())) {
                const newCoefficient = simplify(Sum.of(monos1[lastInserted].coefficient, m.coefficient));
                if (newCoefficient.identical(Integer.zero)) {
                    ...
                    monos1.splice(
                        lastInserted,
                        1,
                        new Monomial(newCoefficient, [...m.variables], [...m.exponentVector]),
                    );
                }
            } else if (monos1[lastInserted - 1] && monos1[lastInserted - 1].body().identical(m.body())) {
                const newCoefficient = simplify(Sum.of(monos1[lastInserted - 1].coefficient, m.coefficient));
                if (newCoefficient.identical(Integer.zero)) {
                    monos1.splice(--lastInserted, 1);
                } else {
                    monos1.splice(
