...
function simplifySum(e: Sum): Expression {
    ...
    for (let i = 0; i < simpleAddends.length; i++) {
        ...
        if (simpleAddends[i] instanceof Quotient) {
            ...
            simpleAddends.splice(i, 1);
            return simplify(
                new Quotient(Sum.of(numerator, Product.of(denominator, Sum.of(...simpleAddends))), denominator),
            );
        }
    }
    ...
    for (let i = 0; i < standardAddends.length; i++) {
        const expression_i = Product.of(...(standardAddends[i] as Product).factors.slice(1));
        for (let j = i + 1; j < standardAddends.length; j++) {
            const expression_j = Product.of(...(standardAddends[j] as Product).factors.slice(1));
...
function simplifyProduct(e: Product): Expression {
    ...
    //sums
    const sumFactors: Sum[] = simpleFactors.filter((s) => s instanceof Sum) as Sum[];
...
function simplifyQuotient(e: Quotient): Expression {
    ...
    if (n.length > 0 && d.length > 0 && d[0] instanceof Integer) {
        const numeratorConstant = n[0] instanceof Integer ? (n[0] as Integer).int : BigInt(1);
        const g: bigint =
            (numericGCD(numeratorConstant, (d[0] as Integer).int) as bigint) * BigInt((d[0] as Integer).signum());
    ...
    for (let i = 0; i < n.length; i++) {
        for (let j = 0; j < d.length; j++) {
            if (equals((n[i] as Power).base, (d[j] as Power).base)) {
                n[i] = new Power(
                    (n[i] as Power).base,
                    Sum.difference((n[i] as Power).exponent, (d[j] as Power).exponent),
                );
                d.splice(j--, 1);
            }
        }
    ...
    if (simpleNumerator instanceof Quotient) {
        return simplify(
            new Quotient(simpleNumerator.numerator, Product.of(simpleNumerator.denominator, simpleDenominator)),
        );
    }
...
function simplifyPower(e: Power): Expression {
    ...
    if (simpleBase instanceof Integer && simpleExponent instanceof Fraction) {
        ...
        const sqrt = simpleBase.powerDecimal(new Decimal(simpleExponent.numeric())).numeric();
        if (Math.floor(sqrt) == sqrt) {
    ...
    if (simpleBase instanceof Sum && simpleExponent instanceof Integer) {
        ...
        const addends = [];
        while (loop) {
            const sum = exponentVector.reduce((s, v, i) => (i < k - 1 ? s + v : s));
            exponentVector[k - 1] = n - sum;

            const factors: Expression[] = [new Integer(multinomialCoefficient(exponentVector))];
...
export function sumOrder(e1: Expression, e2: Expression): number {
    ...
    if (e1 instanceof Product || e2 instanceof Product) {
        ...
        const ownHasNumericExponents = own.factors.every((f, i) => i == 0 || (f as Power).exponent instanceof Number);
        ...
        if (ownHasNumericExponents && otherHasNumericExponents) {
            const ownExpSum = own.factors
                .filter((v, i) => i > 0)
                .reduce((sum, v) => (sum += ((v as Power).exponent as Number).numeric()), 0);
