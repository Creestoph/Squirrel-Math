        ...
        if (m1.variables.length > 0 && m2.variables.length > 0) {
            ...
            while (i < m1.variables.length && j < m2.variables.length) {
                if (productOrder(m2.variables[j], m1.variables[i]) < 0) {
                    ...
                    exponents.push(
                        new Integer(Math.min(m1.exponentVector[i].numeric(), m2.exponentVector[j].numeric())),
                    );
                    i++;
                    j++;
                }
...
export const monomialGLexOrder = new Order((m1: Monomial, m2: Monomial) => {
    m1 = m1.cleaned();
    m2 = m2.cleaned();
    const m1ExpSum = m1.exponentVector.reduce((sum, v) => (sum += v.numeric()), 0);
    const m2ExpSum = m2.exponentVector.reduce((sum, v) => (sum += v.numeric()), 0);
            ...
            if (
                divisor.variables.every((v, i) =>
                    divident.variables.some(
                        (v2, j) => v.identical(v2) && !divident.exponentVector[j].lessThan(divisor.exponentVector[i]),
                    ),
                )
            ) {
...
export function polynomialDivision(
    ...
    while (remainder.degree() >= divider.degree()) {
        ...
        const q = Product.of(
            simplify(new Quotient(remainder.leadingCoefficient(), divider.leadingCoefficient())),
            new Power(x, new Integer(remainder.degree() - divider.degree())),
        );
        remainder = UnivariatePolynomial.fromPolynomial(
            Polynomial.asPolynomial(simplify(Sum.difference(remainder, Product.of(q, divider)))),
            x,
        );
        resultAddends.push(Monomial.asMonomial(q));
    }
    return {
        quotient: UnivariatePolynomial.fromPolynomial(Polynomial.fromMonomials(resultAddends), x),
        remainder: remainder,
    };
}
...
export function extractContent(p: Polynomial) {
    ...
    const commonDenominator = denominators.length ? LCM(...denominators) : Integer.one;
...
export function grobnerBasis(
    ...
    while (changed) {
        ...
        for (let i = 0; i < result.length && !changed; i++) {
            ...
            const interreduced = multivariateDivision(result[i], allExceptThis, order, variables).remainder;
            ...
            for (let p = 0; p < pairsToTry.length && !changed; p++) {
                ...
                for (let k = 0; k < result.length && !existsK; k++) {
                    if (
                        ...
                        k != pairsToTry[p][1] &&
                        !pairsToTry.some(
                            (pp) => pp[0] == Math.min(k, pairsToTry[p][0]) && pp[1] == Math.max(k, pairsToTry[p][0]),
                        ) &&
                        !pairsToTry.some(
                            (pp) => pp[0] == Math.min(k, pairsToTry[p][1]) && pp[1] == Math.max(k, pairsToTry[p][1]),
                        ) &&
                ...
                if (!equals(leadingLcmBody, Product.of(leadingMonomial1, leadingMonomial2)) && !existsK) {
                    const co1 = Monomial.asMonomial(simplify(new Quotient(leadingLcmBody, leadingMonomial1))).multiply(
                        p2.monomials[0].coefficient,
                    );
                    const co2 = Monomial.asMonomial(simplify(new Quotient(leadingLcmBody, leadingMonomial2))).multiply(
                        Product.of(p1.monomials[0].coefficient, Integer.minusOne),
                    );
...
export function factorizePolynomial(p: Polynomial): Product {
    ...
    if (!equals(extranction.factors[0], Integer.one)) {
        ...
        if (extranction.factors[0] instanceof Fraction) {
            constant = [
                new Fraction(1, (extranction.factors[0] as Fraction).denominator.int),
                ...factorizeNumber((extranction.factors[0] as Fraction).numerator.int)
                    .filter((f) => f != BigInt(1))
                    .map((f) => new Integer(f)),
            ];
    ...
    for (let v = 0; v < p.variables.length; v++) {
        ...
        const leading = factorizePolynomial(
            Polynomial.asPolynomial(simplify(pv.coefficients[pv.coefficients.length - 1])),
        );
        const free = factorizePolynomial(Polynomial.asPolynomial(simplify(pv.coefficients[0])));
                ...
                const valueTest = Sum.of(
                    ...pv.coefficients.map((c, exp) =>
                        Product.of(
                            ...
                            new Power(leadingDivisors[i], new Integer(pv.degree() - exp)),
                        ),
                    ),
                );
                if (equals(valueTest, Integer.zero)) {
                    const divisorPolynomial = UnivariatePolynomial.withCoefficients(
                        [Product.of(Integer.minusOne, freeDivisors[j]), leadingDivisors[i]],
                        p.variables[v],
                    );
                    const { quotient } = polynomialDivision(pv, divisorPolynomial);
...
export function factorize(e: Expression): Product {
    ...
    standard.addends.forEach((a) =>
        (a as Product).factors
            .filter(
                (f, i) => i > 0 && (f as Power).exponent instanceof Integer && (f as Power).base instanceof Variable,
            )
