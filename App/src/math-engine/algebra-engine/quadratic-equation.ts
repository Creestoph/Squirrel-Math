...
export class QuadraticEquation extends Equation {
    ...
    solve(): Expression[] {
        ...
        if (isInequalityIdentity(new Inequality(delta, InequalitySign.LESS, Integer.zero))) return [];
        else if (equals(delta, Integer.zero)) return [new Quotient(Product.of(Integer.minusOne, this.b), denominator)];
        else
            return [
                new Quotient(
                    Sum.of(Product.of(Integer.minusOne, this.b), new Power(delta, new Fraction(1, 2))),
                    denominator,
                ),
                new Quotient(
                    Sum.difference(Product.of(Integer.minusOne, this.b), new Power(delta, new Fraction(1, 2))),
                    denominator,
                ),
            ];
    }
