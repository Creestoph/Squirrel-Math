...
export default class TextAlign extends Extension {
    ...
    get commands() {
        return () => (newAlign: string) => (state: EditorState, dispatch: any) => {
            if (dispatch) {
                    ...
                    state.doc.nodesBetween(from, to, (node, pos) => {
                        if (node.type.name === 'paragraph') {
                            transaction.setNodeMarkup(pos, undefined, {
                                ...node.attrs,
                                textAlign: TextAlign.alignToAttribute(newAlign),
                            });
                        }
                    });
                });
    ...
    isActive(align: string) {
        ...
        this.editor.state.tr.selection.ranges.forEach((range: any) => {
            ...
            this.editor.state.doc.nodesBetween(from, to, (node: any) => {
                found =
                    found ||
                    (node.type.name === 'paragraph' && node.attrs.textAlign == TextAlign.alignToAttribute(align));
            });
        });
        return found;
    }
