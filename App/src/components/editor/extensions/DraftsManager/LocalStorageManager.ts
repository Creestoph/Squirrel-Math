...
export class LocalStorageSaver {
    ...
    static loadDraft(draft: DraftPreview): Draft {
        return Object.assign(draft, {
            lesson: JSON.parse(
                localStorage.getItem(LocalStorageSaver.prefix + (draft.fromAutosave ? 'autosave-' : '') + draft.name)!,
            ),
        });
    }
    ...
    static deleteDraft(draft: DraftPreview): void {
        ...
        localStorage.removeItem(
            LocalStorageSaver.prefix + 'metadata-' + (draft.fromAutosave ? 'autosave-' : '') + draft.name,
        );
    }
    ...
    static saveDraft(draft: Draft, fromAutosave: boolean) {
        ...
        localStorage.setItem(
            LocalStorageSaver.prefix + (fromAutosave ? 'autosave-' : '') + draft.name,
            JSON.stringify(draft.lesson),
        );
        localStorage.setItem(
            LocalStorageSaver.prefix + 'metadata-' + (fromAutosave ? 'autosave-' : '') + draft.name,
