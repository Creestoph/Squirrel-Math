...
class Columnar_division_step {
    private readonly table: Display_table;
...
export class Columnar_division {
    private steps: Columnar_division_step[] = [];
    private step = 0;
    ...
    private generate_steps(numbers: string[], is_float = true) {
        ...
        let standard_err = 'Wpisz dwie liczby do podzielenia <br>np. 1234:73';
        if (!is_float) {
            standard_err = 'Wpisz dwie liczby naturalne do podzielenia <br>np. 1234:73';
        }
        ...
        let decmial_places_1 = 0;
        for (let i = 0; i < numbers[1].length; i++) {
            if (numbers[1][i] == ',') {
                decmial_places_1 = numbers[1].length - 1 - i;
            }
        }
        let decmial_places_0 = 0;
        for (let i = 0; i < numbers[0].length; i++) {
            if (numbers[0][i] == ',') {
                decmial_places_0 = numbers[0].length - 1 - i;
            }
        }
        const decimal_diff = decmial_places_0 - decmial_places_1;
        numbers[0] = numbers[0].replace(',', '');
        if (decimal_diff < 0) {
            for (let i = 0; i < -1 * decimal_diff; i++) {
                numbers[0] += '0';
            }
        } else if (decimal_diff > 0) {
            numbers[0] =
                numbers[0].slice(0, numbers[0].length - decimal_diff) +
                ',' +
                numbers[0].slice(numbers[0].length - decimal_diff);
        }
        ...
        if (decmial_places_1 > 0) {
            ...
            if (decmial_places_1 == 1) {
                ...
                (decmial_places_1 % 10 == 2 || decmial_places_1 % 10 == 3 || decmial_places_1 % 10 == 4) &&
                decmial_places_1 / 10 != 1
            ) {
                comment += ' cyfry';
            } else {
                comment += ' cyfr';
            }
            comment += ' po przecinku,' + ' przesuwamy przecinek dzielnej o ' + decmial_places_1 + ' w prawo. ';
        }
        comment += 'Zapisujemy obie liczby obok siebie i rysujemy kreskę ponad nimi.';
        let highlight_fields = Columnar_division.empty_highlight(table);
        ...
        for (let h = 0; h < numbers[1].length; h++) {
            highlight_fields[1][numbers[0].length + 1 + h] = true;
        }
        this.steps.push(new Columnar_division_step(table, highlight_fields, comment));
        ...
        for (; fi < numbers[0].length; fi++) {
            ...
            for (let j = i; j >= 0 && table[table.length - 1][j] != ''; j--) {
                highlight_fields[table.length - 1][j] = true;
            }
            for (let h = 0; h < numbers[1].length; h++) {
                highlight_fields[1][numbers[0].length + 1 + h] = true;
            }
            ...
            highlight_fields = Columnar_division.empty_highlight(table);
            for (let j = z.length - 1; j >= 0; j--) {
                highlight_fields[table.length - 1][i - (z.length - 1 - j)] = true;
            }
            for (let j = y.length - 1; j >= 0; j--) {
                highlight_fields[table.length - 2][i - (y.length - 1 - j)] = true;
            }
            for (let j = x.length - 1; j >= 0; j--) {
                highlight_fields[table.length - 3][i - (x.length - 1 - j)] = true;
            }
            //for (let h = 0; h < numbers[1].length; h++) highlight_fields[1][numbers[0].length +1+h] = true;
            this.steps.push(new Columnar_division_step(table, highlight_fields, comment));
            i++;
        ...
        if (parseInt(z) == 0) {
            ...
            let from_end = occured.length - occured.indexOf(n);
            if (from_end == 0) {
                from_end = 1;
            }
            result = result.slice(0, result.length - from_end) + '(' + result.slice(result.length - from_end) + ')';
            comment += (result[0] == ',' ? '0' : '') + result + '.';
            let temp = table[0][table[0].length - from_end - 1];
            ...
            for (let h = 0; h < highlight_fields[highlight_fields.length - from_end * 2 - 1].length; h++) {
                if (table[highlight_fields.length - from_end * 2 - 1][h].toString() != '') {
                    highlight_fields[highlight_fields.length - from_end * 2 - 1][h] = true;
                }
            }
            this.steps.push(new Columnar_division_step(table, highlight_fields, comment));
            return;
    ...
    private static add_empty_row(tab: string[][]) {
        const tab1: string[][] = [];
        for (let i = 0; i < tab.length; i++) {
            tab1[i] = [];
            for (let j = 0; j < tab[i].length; j++) {
    ...
    private static add_empty_column(tab: string[][]) {
        const tab1: string[][] = [];
        for (let i = 0; i < tab.length; i++) {
            tab1[i] = [];
            for (let j = 0; j < tab[i].length; j++) {
