...
const renderElem = function(elem: HTMLElement, optionsCopy: any) {
    for (let i = 0; i < elem.childNodes.length; i++) {
        const childNode = elem.childNodes[i] as HTMLElement;
        if (childNode.nodeType === 3) {
            // Text node
            // Concatenate all sibling text nodes.
            // Webkit browsers split very large text nodes into smaller ones,
            // so the delimiters may be split across different nodes.
            let textContentConcat: string = childNode.textContent ?? '';
            let sibling: HTMLElement | null = childNode.nextSibling as HTMLElement;
            let nSiblings = 0;
            ...
            const shouldRender = optionsCopy.ignoredTags.indexOf(
                childNode.nodeName.toLowerCase()) === -1 &&
                  optionsCopy.ignoredClasses.every(
                      (x: string) => className.indexOf(' ' + x + ' ') === -1);
...
const renderMathInElement = function(elem: HTMLElement, options: any) {
    if (!elem) {
