...
export function numberToStr(input: string) {
    ...
    if (digits.some((digit) => isNaN(digit))) return 'Nie umiesz wpisać liczby naturalnej?';
    ...
    for (let i = digits.length - 1; i >= 0; i -= 3) {
        ...
        if (order >= orders.length) return 'To dla mnie za dużo. Daj mi spokój.';
        let groupString = '';
        if (group === 1 && order > 0) groupString = orders[order];
            ...
            if (units < 5 && units > 1 && tens !== 1) groupString += ' ' + ordersPlural[order];
            else if (group > 0) groupString += ' ' + ordersPluralGenitive[order];
        }
