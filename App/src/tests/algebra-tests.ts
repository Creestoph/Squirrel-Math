    ...
    it(equations.join(' and ') + ' in variables ' + variables.join(', ') + ' give solution: ' + s, () => {
        ...
        expect(
            calculated.length == s.length &&
                calculated.every((c) => expected.some((e) => c.every((cc, i) => equals(cc, e[i])))),
        ).to.be.true;
    });
}
    ...
    it(equation + ' solutions: ' + s, () => {
        ...
        expect(
            calculated.elements.every(
                (c) =>
                    c instanceof Number &&
                    expected.elements.some((e) => Math.abs((e as Number).numeric() - c.numeric()) < precision),
            ),
        ).to.be.true;
        expect(
            expected.elements.every((e) =>
                calculated.elements.some(
                    (c) => Math.abs((e as Number).numeric() - (c as Number).numeric()) < precision,
                ),
            ),
        ).to.be.true;
    });
        ...
        expect(
            equals(
                multivariateDivision(
                    ...
                    base.map((s) => Polynomial.asPolynomial(parseExpression(s))),
                    monomialLexOrder,
                    vars,
                ).remainder,
        ...
        () => {
            ...
            expect(
                grobnerBasis(poly, o, vars).every((g) =>
                    ex.some((e) => simplify(new Quotient(g, e)) instanceof Number),
                ),
            ).to.be.true;
        },
    );
