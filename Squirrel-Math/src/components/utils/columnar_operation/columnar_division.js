...
Columnar_division_step.prototype.print = function (
    coment_target_id,
    table_target_id,
) {
...
export class Columnar_division {
    ...
    generate_steps(numbers, is_float = true) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            for (
                let j = 0;
                j < numbers[1].length + numbers[0].length + 1;
                j++
            ) {
                table[i][j] = '';
            }
        ...
        if (decmial_places_1 > 0) {
            comment +=
                ...
                ' ma ' +
                decmial_places_1;
            if (decmial_places_1 == 1) comment += ' cyfrę';
            else if (
                (decmial_places_1 % 10 == 2 ||
                    decmial_places_1 % 10 == 3 ||
                    decmial_places_1 % 10 == 4) &&
            ...
            comment +=
                ' po przecinku,' +
                ' przesuwamy przecinek dzielnej o ' +
                decmial_places_1 +
                ' w prawo. ';
        }
        comment +=
        ...
        this.steps.push(
            new Columnar_division_step(table, highlight_fields, comment),
        );
        ...
        for (var h = 0; h < numbers[1].length; h++)
        ...
        for (; fi < numbers[0].length; fi++) {
            ...
            var y = parseInt(
                parseInt(table[0][fi]) * parseInt(numbers[1]),
            ).toString();
            table = Columnar_division.add_empty_row(table);
            ...
            highlight_fields = Columnar_division.empty_highlight(table);
            for (let j = z.length - 1; j >= 0; j--)
                highlight_fields[table.length - 1][i - (z.length - 1 - j)] =
                    true;
            for (let j = y.length - 1; j >= 0; j--)
                highlight_fields[table.length - 2][i - (y.length - 1 - j)] =
                    true;
            for (let j = x.length - 1; j >= 0; j--)
                highlight_fields[table.length - 3][i - (x.length - 1 - j)] =
                    true;
            //for (var h = 0; h < numbers[1].length; h++) highlight_fields[1][numbers[0].length +1+h] = true;
            this.steps.push(
                new Columnar_division_step(table, highlight_fields, comment),
        ...
        if (parseInt(z) == 0) {
            ...
            while (occured.indexOf(n) == -1) {
                ...
                if (parseInt(z) == 0) {
                    comment =
                        'Ponieważ w wyniku odejmowania otrzymaliśmy 0, kończymy procedurę. Odcztujemy wynik ';
                    let zeros = 1;
                    let result = '';
                    for (let j = 0; j < table[0].length; j++) {
                        if (table[0][j] == '0' && zeros != 1)
            ...
            result =
                result.slice(0, result.length - from_end) +
                '(' +
                result.slice(result.length - from_end) +
                ')';
            comment += (result[0] == ',' ? '0' : '') + result + '.';
            ...
            for (
                let h = 0;
                h <
                highlight_fields[highlight_fields.length - from_end * 2 - 1]
                    .length;
                h++
            )
                if (
                    table[highlight_fields.length - from_end * 2 - 1][
                        h
                    ].toString() != ''
                )
                    highlight_fields[
                        highlight_fields.length - from_end * 2 - 1
                    ][h] = true;
            this.steps.push(
                new Columnar_division_step(table, highlight_fields, comment),
            );
