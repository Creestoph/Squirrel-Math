...
function Columnar_multiplication_step(
    table,
    highlight_fields,
    comment,
    carry,
...
Columnar_multiplication_step.prototype.print = function (
    coment_target_id,
    table_target_id,
) {
...
export class Columnar_multiplication {
    ...
    generate_steps(numbers, is_float = true) {
        ...
        if (
            numbers[0].length + numbers[1].length > 39 ||
            numbers[1].length > 10
        ) {
        ...
        this.steps.push(
            new Columnar_multiplication_step(
                table,
                highlight_fields,
                comment,
                carry,
        ...
        for (let i = numbers[1].length - 1; i >= 0; i--) {
            ...
            comment =
                'Przystępujemy do mnożenia liczby ' +
                numbers[0] +
                ' przez ' +
                numbers[1][i] +
                '.';
            ...
            for (let j = numbers[0].length - 1; j >= 0; j--) {
                ...
                if (cj == 0) {
                    if (ci == 1) comment += ' z pojedynczym wcięciem';
                    if (ci == 2) comment += ' z podwójnym wcięciem';
                    if (ci == 3) comment += ' z potrójnym wcięciem';
                    if (ci > 3) comment += ' z odpowiednim wcięciem';
                }
                if (carry > 0) comment += ', a ' + carry + ' zapamiętujemy.';
                else comment += '.';
                highlight_fields =
                    Columnar_multiplication.empty_highlight(table);
            ...
            if (carry != 0) {
                ...
                } else table[2 + (numbers[1].length - 1 - i)][t - 1] = carry;
        ...
        if (numbers[1].length == 1) {
            ...
            for (let i = table[0].length - 1; i >= 0; i--) {
                ...
                for (let j = 2; j < table.length; j++) {
                    sum =
                        parseInt(sum) +
                        parseInt(table[j][i] == '' ? '0' : table[j][i]);
                }
                w = w + (sum % 10).toString();
            ...
            comment =
                'Otrzymane liczby podkreślamy i wykonujemy ich dodawanie pisemne.';
            highlight_fields = Columnar_multiplication.empty_highlight(table);
            this.steps.push(
                new Columnar_multiplication_step(
                    table,
        ...
        if (commas[0] + commas[1] == 0)
            ...
            comment +=
                ' po przecinku, a ' +
                numbers_orig[1].replace('.', ',') +
                ' ma ' +
                commas[1] +
                ' ';
            if (commas[1] == 1) comment += 'cyfrę';
            else if (
                commas[1] / 10 != 1 &&
                (commas[1] % 10 == 2 ||
                    commas[1] % 10 == 3 ||
