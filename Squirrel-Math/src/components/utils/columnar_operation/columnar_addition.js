...
Columnar_addition_step.prototype.print = function (
    coment_target_id,
    table_target_id,
) {
    ...
    generate_steps(numbers, is_float = true) {
        ...
        for (let i = 0; i < numbers.length; i++) {
            ...
            for (var k = 0; k < numbers[i].length; k++) {
                table[i + 1][longest_before_comma - beforeComma + k] =
                    numbers[i][k];
            }
        }
        this.steps.push(
        ...
        comment += '.';
        this.steps.push(
            new Columnar_addition_step(table, longest_after_comma, -1, comment),
        );
    }
}
