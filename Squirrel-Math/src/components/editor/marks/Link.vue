<template>
    <span class="link-wrapper">
        <div class="link-editor" v-if="showEditor">
            ...
            <button class="navigate-button" @click="navigate()">
                Odwiedź stronę
            </button>
    ...
    mounted() {
        if (this.href === '') this.openPopup();
            ...
            this.getLessons().then(
                (lessons) =>
                    (this.selectedLesson = lessons.find(
                        (l) => '/lesson/' + l.title == lessonUrl,
                    ).title),
            );
        }
    },
    methods: {
        getLessons() {
            if (this.lessons.length == 1) {
                return import(`@/assets/current_lesson_graph.json`).then(
                    (file) => {
        ...
        getChapters() {
            if (this.selectedLesson) {
                import(`@/assets/lessons/${this.selectedLesson}.json`).then(
                    (file) => {
                        const chapters = file.long.content.filter(
                            (c) => c.type == 'chapter',
                        );
                        if (chapters.length)
        ...
        selectLesson(lesson) {
            ...
            this.url =
                '/lesson/' + this.lessons.find((l) => l.title == lesson).title;
        ...
        selectChapter(chapter) {
            ...
            this.url =
                '/lesson/' +
                this.lessons.find((l) => l.title == this.selectedLesson).title +
                '#' +
                chapter;
        },
        openPopup() {
