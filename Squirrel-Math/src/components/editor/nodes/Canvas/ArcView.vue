...
export default {
    ...
    computed: {
        ...
        angle: {
            get() {
                return (
                    Math.round(
                        ((this.arm2.angle - this.arm1.angle + 360) % 360) * 100,
                    ) / 100
                );
            },
            ...
            set(value) {
                this.arm2.angle =
                    Math.round(
                        ((parseFloat(value) || 0) + this.arm1.angle) * 100,
                    ) / 100;
                this.line2.segments[1].point = this.grips.children[2].position =
                    this.arm2.add(this.center);
                this.save();
            },
        },
        ...
        center: {
            get() {
                return this.line1.segments[0].point;
            },
        },
        arm1End: {
            get() {
                return this.line1.segments[1].point;
            },
        },
        arm2End: {
            get() {
                return this.line2.segments[1].point;
            },
        },
        arm1: {
            get() {
                return this.arm1End.subtract(this.center);
            },
        },
        arm2: {
            get() {
                return this.arm2End.subtract(this.center);
            },
        },
    },
    ...
    methods: {
        render() {
            if (this.paperScope) this.paperScope.project.clear();
            ...
            this.line1 = new paper.Path([
                new paper.Point(attrs.center),
                new paper.Point(attrs.arms[0]),
            ]);
            this.line2 = new paper.Path([
                new paper.Point(attrs.center),
                new paper.Point(attrs.arms[1]),
            ]);
            this.line1.style.strokeWidth = this.line2.style.strokeWidth = 3;
            this.line1.strokeColor = this.line2.strokeColor = new paper.Color(
                0.8,
                0.8,
                0.8,
            );
        ...
        containedInBounds(bounds) {
            return (
                this.arcFill.intersects(new paper.Path.Rectangle(bounds)) ||
                bounds.contains(this.arcFill.bounds)
            );
        },
        ...
        onMouseMove(event, hitResult, cursorStyle) {
            if (
                hitResult &&
                (this.arcStroke == hitResult.item ||
                    this.grips.children.some((item) => item == hitResult.item))
            )
                cursorStyle.cursor = 'crosshair';
            else if (
                hitResult &&
                (this.line1 == hitResult.item ||
                    this.line2 == hitResult.item ||
                    this.arcFill == hitResult.item)
            )
        ...
        onMouseDrag(event, snapPoints) {
            ...
            if (this.movedShape == 'arc') {
                ...
                if (
                    (this.arm1.angle < this.arm2.angle &&
                        (mouseArm.angle < this.arm1.angle ||
                            mouseArm.angle > this.arm2.angle)) ||
                    (this.arm1.angle > this.arm2.angle &&
                        mouseArm.angle > this.arm2.angle &&
                        mouseArm.angle < this.arm1.angle)
                ) {
                    ...
                    this.line1.segments[1].point =
                        this.line2.segments[1].point.clone();
                    this.line2.segments[1].point = swapTemp;
                    this.save();
                } else this.recalculateArc();
                return true;
            }
        ...
        save() {
            this.updateAttrs({
                ...
                arms: [
                    ...
                    { x: this.arm2End.x, y: this.arm2End.y },
                ],
                radius: this.radius,
            });
        },
        ...
        recalculateArc() {
            ...
            let mid = this.arm1End
                .add(this.arm2End)
                .multiply(0.5)
                .subtract(this.center);
            if (this.angle > 180) mid = mid.multiply(-1);
            mid = mid.add(
                this.arm1End.subtract(this.arm2End).rotate(90).normalize(1),
            );
