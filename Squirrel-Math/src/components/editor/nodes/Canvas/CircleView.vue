...
export default {
        ...
        borderColor: {
            ...
            set(borderColor) {
                ...
                this.circle.style.strokeWidth =
                    this.circle.strokeColor.alpha > 0 ? 3 : 0;
    ...
    methods: {
        ...
        onMouseMove(event, hitResult, cursorStyle) {
            if (!hitResult) return;
            else if (this.circle == hitResult.item) cursorStyle.cursor = 'move';
            else if (
        ...
        onMouseDrag(event, snapPoints) {
            ...
            this.circle.bounds.width =
                this.right.position.x - this.left.position.x;
            this.circle.bounds.height =
        ...
        recalculateGripsPositions() {
            this.upper.position = this.circle.position.add(
                new paper.Point(0, -this.circle.bounds.height / 2),
            );
            this.bottom.position = this.circle.position.add(
                new paper.Point(0, this.circle.bounds.height / 2),
            );
            this.left.position = this.circle.position.add(
                new paper.Point(-this.circle.bounds.width / 2, 0),
            );
            this.right.position = this.circle.position.add(
                new paper.Point(this.circle.bounds.width / 2, 0),
            );
            this.upperLeft.position = this.circle.position.add(
                new paper.Point(
                    -this.circle.bounds.width / 2,
