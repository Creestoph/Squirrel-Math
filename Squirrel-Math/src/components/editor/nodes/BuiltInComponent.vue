            ...
            <div class="form">
                <div class="form-header">
                    ...
                    >
                        ...
                        <template v-if="componentName == 'other'">
                            <option
                                v-for="(component, name) in builtInComponents"
                                :key="name"
                                :value="name"
                            >
                                {{ component.name }}
                        ...
                        <template v-if="componentName != 'other'">
                            <option
                                v-for="(component, name) in availableComponents"
                                :key="name"
                                :value="name"
                            >
                                {{ component.name }}
                ...
                <div class="form-body" v-if="componentName">
                    <div v-if="!productionMode || componentName != 'other'">
                        <div
                            v-for="(
                                parameterSchema, parameterName, i
                            ) in builtInComponents[componentName].schema"
                            :key="i"
                            class="form-row"
                        >
                            <div class="form-col">
                                {{
                                    builtInComponents[componentName].labels[
                                        parameterName
                                    ]
                                }}
                            </div>
                            <div class="form-col">
                                ...
                                <input
                                    v-if="
                                        parameterSchema.type.name == 'BOOLEAN'
                                    "
                                    :required="parameterSchema.required"
                                    type="checkbox"
                                    v-model="formArgs[i]"
                                ...
                                <input
                                    v-if="
                                        parameterSchema.type.name == 'FUNCTION'
                                    "
                                    :required="parameterSchema.required"
                                    type="function"
                                    v-model="formArgs[i]"
                                ...
                                />
                                <div
                                    v-if="parameterSchema.type.name == 'ARRAY'"
                                >
                                    <input
                                        v-for="(arg, j) in formArgs[i]"
                                        :key="j"
                                        :required="
                                            parameterSchema.required && j == 0
                                        "
                                        type="array"
                                    ...
                                    />
                                    <button
                                        @click="addElementForArrayParameter(i)"
                                        class="array-parameter-button"
                                    >
                                        + element
                                    </button>
                                    <button
                                ...
                                >
                                    <option
                                        v-for="(value, j) in parameterSchema
                                            .type.values"
                                        :key="j"
                                        :value="value"
                                    >
                    ...
                    <div v-if="productionMode && componentName == 'other'">
                        <comment text="Leniwi programiści."
                            >Ten komponent podlega edycji tylko przez
                            administrację Squirrel-Math.</comment
                        >
                    </div>
                </div>
            </div>
...
export default {
    ...
    mounted() {
        ...
        const configNonEmpty = this.formArgs.some((arg, i) => {
            const argType = Object.values(
                this.builtInComponents[this.componentName].schema,
            )[i].type.name;
            return (
                ((argType == 'TEXT' ||
                    argType == 'ENUM' ||
        ...
        run() {
            ...
            Object.entries(
                ...
                if (
                    ...
                    try {
                        this.componentConfiguration[key] = eval(
                            this.formArgs[i],
                        );
                        if (
                            typeof this.componentConfiguration[key] !=
                            'function'
                        )
                            throw '';
                    } catch (e) {
