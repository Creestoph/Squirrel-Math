<template>
    <div class="tags-wrapper">
        <div class="dropdown" v-for="(tag, j) in tags" :key="'tag' + j">
            <div class="type">
                {{ tag }}
                <span v-if="tags.length > 1" @click="removeTag(j)" class="cross"
                    >⨯</span
                >
            </div>
        ...
        <span v-if="canAddTag()" @click="addTag()" class="add-tag-button"
            >+</span
        >

        <span
            :class="
                required.length == 0 ? 'required-optional' : 'required-strong'
            "
            >Wymagane:</span
        >
        <div
        ...
        <span
            class="add-required-button"
            v-if="canAddNewRequired()"
            @click="addRequiredLesson()"
            >+</span
        >
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        availableLessons: {
            get() {
                return this.allLessons.filter(
                    (lesson) => !this.required.includes(lesson.title),
                );
            },
        },
    },
        ...
        canAddNewRequired() {
            return (
                (!this.required.length ||
                    this.required[this.required.length - 1]) &&
                this.availableLessons.length > 0
            );
        },
    },
};
