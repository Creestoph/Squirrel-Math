    ...
    import(`@/assets/current_lesson_graph.json`).then((file) => {
        ...
        lessons.forEach(async (lessonSpecification, i) => {
            const lesson = (await import(
                `@/assets/lessons/${lessonSpecification.title}.json`
            )) as LessonData;
            if (lesson.short)
                lesson.short.content.forEach((node) =>
                    transformNodeAndChildren(node),
                );
            if (lesson.long)
                lesson.long.content.forEach((node) =>
                    transformNodeAndChildren(node),
                );
            setTimeout(() => {
                downloadFile(
                    JSON.stringify(lesson),
                    lessonSpecification.title,
                    'application/json',
                );
            }, i * 500);
        });
