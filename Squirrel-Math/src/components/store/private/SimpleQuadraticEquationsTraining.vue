...
export default class SimpleQuadraticEquationsTraining extends Vue {
    ...
    updateProduct() {
        let userX1Numerator,
            ...
            userX2Numerator,
            userX2Denominator;
        [userX1Numerator, userX1Denominator] = this.x1
            .split('/')
            .map((n) => parseInt(n));
        [userX2Numerator, userX2Denominator] = this.x2
            .split('/')
            .map((n) => parseInt(n));
        ...
        if (
            this.x1 == '' ||
            this.x2 == '' ||
            isNaN(parseInt(this.x1)) ||
            isNaN(parseInt(this.x2))
        )
            return;
        ...
        if (
            ...
            product += new Power(
                simplify(
                    UnivariatePolynomial.withCoefficients(
                        [
                            this.userX1.numerator.opposite(),
                            this.userX1.denominator,
                        ],
                        this.varX,
                    ),
                ),
            ...
            product += Product.of(
                ...
                simplify(
                    UnivariatePolynomial.withCoefficients(
                        [
                            this.userX2.numerator.opposite(),
                            this.userX2.denominator,
                        ],
                        this.varX,
                    ),
                ),
    ...
    check() {
        ...
        if (this.x1 == '' || this.x2 == '')
            ...
            (equals(this.userX1, this.correctX1) &&
                equals(this.userX2, this.correctX2)) ||
            (equals(this.userX1, this.correctX2) &&
                equals(this.userX2, this.correctX1))
        )
            result = 'Dobrze!';
        else if (
    ...
    next() {
        let x1Numerator = Integer.random(0, 9).multiply(
            new Integer(Math.random() > 0.6 ? 1 : -1),
        ) as Integer;
        let x2Denominator;
        if (equals(x1Numerator, Integer.zero) || Math.random() < 0.8)
            ...
            do {
                ...
                numericGCD(x1Numerator.int, x2Denominator.int) > BigInt(1)
            );
        ...
        let equationPolynomial = simplify(
            Product.of(
                UnivariatePolynomial.withCoefficients(
                    [
                        this.correctX1.numerator.opposite(),
                        this.correctX1.denominator,
                    ],
                    this.varX,
                ),
                UnivariatePolynomial.withCoefficients(
                    [
                        this.correctX2.numerator.opposite(),
                        this.correctX2.denominator,
                    ],
                    this.varX,
                ),
            ),
