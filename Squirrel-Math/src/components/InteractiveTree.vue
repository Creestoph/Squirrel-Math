...
export default class InteractiveTree extends Vue {
    ...
    save() {
        ...
        for (let name in this.lessons) {
            json.nodes.push([
                name,
                this.positions[name].x,
                this.positions[name].y,
            ]);
        }
        ...
        this.download(
            JSON.stringify(json),
            'current_graph_coordinates.json',
            'application/json',
        );
    }
    ...
    addEventHandlers() {
        ...
        this.mypaper.tool.onMouseMove = (event: paper.ToolEvent) => {
            const hitResult = this.mypaper.project!.hitTest(
                event.point!,
                hitOptions,
            );
            ...
            if (hitResult && hitResult.type == 'stroke') {
                if (this.editMode && hitResult.item!.data.hasStair) {
                    const stairPosition =
                        this.stairsX[hitResult.item!.data.lesson][
                            hitResult.item!.data.req
                        ];
                    if (Math.abs(hitResult.point!.x! - stairPosition) < 5)
                        (this.$refs.canvas as HTMLElement).style.cursor =
                            'ew-resize';
            ...
            if (hitResult && hitResult.type == 'fill') {
                ...
                if (!this.hoveredLesson) {
                    ...
                    for (let req of this.lessons[lessonName].requires) {
                        this.edges[lessonName][req].style!.strokeColor =
                            redColor;
            ...
            if (!this.editMode && this.hoveredLesson) {
                ...
                if ((event as any).event.button === 0)
                    ...
                    let url = this.$router.resolve(
                        '/lesson/' + this.lessons[this.boldLesson].title,
                    );
        ...
        this.mypaper.tool.onMouseDrag = (event: paper.ToolEvent) => {
            ...
            if (this.hoveredStair) {
                ...
                this.stairsX[this.hoveredStair.data.lesson][
                    this.hoveredStair.data.req
                ] = this.hoveredStair.segments![2].point!.x!;
            }
            if (this.hoveredLesson) {
                let clickedTitle = this.hoveredLesson.content!;
                this.hoveredLesson.position = this.hoveredLesson.position!.add(
                ...
                for (let path of Object.values(this.edges[clickedTitle])) {
                    ...
                    if (segments.length == 4) {
                        ...
                        segments[1].point!.x! += deltaX;
                        segments[3].point!.x =
                            (segments[1].point!.x! + segments[4].point!.x!) / 2;
                        segments[2].point!.x =
                            (segments[1].point!.x! + segments[4].point!.x!) / 2;
                    }
                }
                for (let upper of this.lessons[clickedTitle].isRequiredBy) {
                    let segments = this.edges[upper][clickedTitle]
                        .segments as paper.Segment[];
                    if (segments.length == 4) {
        ...
        this.mypaper.tool.onMouseUp = () => {
            ...
            if (this.editMode) {
                ...
                for (let lesson in this.stairsX)
                    for (let req in this.stairsX[lesson]) {
                        this.stairsX[lesson][req] = snap(
                            this.stairsX[lesson][req],
                        );
                    }
                this.displayLessons();
            }
            ...
            if (lesson.requires.length != 1) {
                for (let req of lesson.requires) {
                    if (this.lessons[req].isRequiredBy.length != 1) {
                        ...
                        this.stairsX[lesson.title][req] =
                            (this.positions[lesson.title].x +
                                this.positions[req].x) /
                            2;
                    }
                }
            }
    ...
    displayLessons() {
        ...
        for (let name in this.lessons) {
            let text = new paper.PointText(
                new paper.Point(this.positions[name].x, this.positions[name].y),
            );
            text.content = name;
            ...
            for (const req of this.lessons[name].requires) {
                ...
                if (this.lessons[name].requires.length == 1) {
                    edge.add(
                        new paper.Point(
                            this.positions[name].x,
                            this.positions[name].y + 5,
                        ),
                    );
                    edge.add(
                        new paper.Point(
                            this.positions[name].x,
                            this.positions[req].y - fontSize - 10,
                        ),
                    );
                    edge.add(
                        new paper.Point(
