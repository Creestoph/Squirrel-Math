...
import { Equation, parseEquation } from '../math-engine/algebra-engine/equation';
...
import { multivariateDivision, monomialLexOrder, monomialGLexOrder, monomialGRevLexOrder, grobnerBasis, factorize } from '../math-engine/algebra-engine/algorithms/polynomial-algorithms';
import { equals, simplify, productOrder } from '../math-engine/algebra-engine/algorithms/simplification-algorithm';
import { solveEquation, solveNumeric, solvePolynomialSystem } from '../math-engine/algebra-engine/algorithms/equation-algorithms';
...
function systemSolution(equations: string[], variables: string[], s: string[][]) {
    it(equations.join(" and ") + " in variables " + variables.join(", ") + " give solution: " + s, () => {
        ...
        let calculated = solvePolynomialSystem(equations.map(e => Polynomial.asPolynomial(parseEquation(e).simplified().left)), variables.map(v => parseExpression(v) as Variable));
        expect(calculated.length == s.length && calculated.every(c => expected.some(e => c.every((cc, i) => equals(cc, e[i]))))).to.be.true;
    });
}

function numericSolutions(equation: string, s: string, lowerBoud: number, upperBound: number, step: number, precision: number) {
    it(equation + " solutions: " + s, () => {
        let expected = new FiniteSet(...s.split(",").map(s => parseExpression(s)));
        let calculated = solveNumeric(new Equation(parseExpression(equation), Integer.zero), new Variable("x"), lowerBoud, upperBound, step, precision);
        expect(calculated.elements.every(c => c instanceof Number && 
            expected.elements.some(e => Math.abs((e as Number).numeric() - c.numeric()) < precision))).to.be.true;
        expect(expected.elements.every(e =>
            calculated.elements.some(c => Math.abs((e as Number).numeric() - (c as Number).numeric()) < precision))).to.be.true;
    });
}
...
function derivate(formula: string, expected: string) {
    it(formula + " has derivative " + expected, () => {
        ...
        expect(equals(derivative(new Function(parseExpression(formula), [v]), v).formula, parseExpression(expected))).to.be.true;
    });
}
...
function divisionRemainder(p: string, base: string[], expected: string, variables?: string[]) {
    it(p + " in base " + base + (variables ? " with variables " + variables : "" ) + " is " + expected, () => {
        ...
        expect(equals(multivariateDivision(Polynomial.asPolynomial(parseExpression(p)), 
        base.map(s => Polynomial.asPolynomial(parseExpression(s))), monomialLexOrder, vars).remainder, parseExpression(expected))).to.be.true;
    });
}

function grobner(p: string[], order: string, expected: string[], variables?: string[]) {
    it(p + " generates basis " + expected + " (with " + order + (variables ? " and variables " + variables : "" ) + ")", () => {
        let poly = p.map(s => Polynomial.asPolynomial(parseExpression(s)) as Polynomial);
        let o;
        let ex = expected.map(e => parseExpression(e));
        let vars = variables ? variables.map(v => parseExpression(v) as Variable) : undefined;
        switch(order) {
            case "lex": o = monomialLexOrder; break;
            case "glex": o = monomialGLexOrder; break;
            case "grevlex": o = monomialGRevLexOrder; break;
        }
        expect(grobnerBasis(poly, o, vars).every(g => ex.some(e => simplify(new Quotient(g, e)) instanceof Number))).to.be.true;
    });
}
...
describe('algebraic simplifications', () => {
    ...
    equalTest("(a+b)/(b+c)+(b+c)/(c+d)", "(ac+ad+bd+b^2+3bc+c^2)/(bc+bd+c^2+cd)");
...
describe('numeric equations', () => {
    ...
    numericSolutions("x^10-2x^8+5x^4-10", "-1.26044, 1.26044", -100, 100, 1, 0.0001);
...
describe('linear systems', () => {
    ...
    systemSolution(["y_1=ax_1+b", "y_2=ax_2+b"], ["a", "b"], [["(y_2-y_1)/(x_2-x_1)", "(x_2y_1-x_1y_2)/(x_2-x_1)"]]);
    systemSolution(["y=a_1x+b_1", "y=a_2x+b_2"], ["x", "y"], [["(b_1-b_2)/(a_2-a_1)", "(-a_1b_2 + a_2b_1)/(a_2-a_1)"]]);
    systemSolution(["x+y+z=1", "x-2y=3", "y-2z=2"], ["x", "y", "z"], [["17/7", "-2/7", "-8/7"]]);
})

describe('polynomial systems', () => {
    systemSolution(["x^2=4", "y^2=9"], ["x", "y"], [["2", "-3"], ["-2", "-3"], ["2", "3"], ["-2", "3"]]);
    systemSolution(["x^2-4y=0", "x-4xy=0"], ["x", "y"], [["0", "0"], ["1", "1/4"], ["-1", "1/4"]]);
    systemSolution(["x^3-y+1=0", "x^3y-x^2y=0"], ["x", "y"], [["-1", "0"], ["0", "1"], ["1", "2"]]);
})
...
describe('polynomials factorization', () => {
    ...
    factors("a^3+3a^2b+3a^2c+3ab^2+6abc+3ac^2+b^3+3b^2c+3bc^2+c^3", "a+b+c,a+b+c,a+b+c");
    factors("x^3-3x^2+3x-1", "x-1,x-1,x-1");
    factors("a^2+2ab+b^2+c^2+2cd+d^2", "a^2+2ab+b^2+c^2+2cd+d^2");
});

describe('monomial orderings', () => {
    orderedPolynomial("1+x+y+z+x^2+y^2+z^2+xy+xz+yz+x^3+y^3+z^3+x^2y+xy^2+x^2z+xz^2+y^2z+yz^2+xyz", monomialLexOrder,     "x^3 + x^2y + x^2z + x^2 + xy^2 + xyz + xy + xz^2 + xz + x + y^3 + y^2z + y^2 + yz^2 + yz + y + z^3 + z^2 + z + 1");
    orderedPolynomial("1+x+y+z+x^2+y^2+z^2+xy+xz+yz+x^3+y^3+z^3+x^2y+xy^2+x^2z+xz^2+y^2z+yz^2+xyz", monomialGLexOrder,    "x^3 + x^2y + x^2z + xy^2 + xyz + xz^2 + y^3 + y^2z + yz^2 + z^3 + x^2 + xy + xz + y^2 + yz + z^2 + x + y + z + 1");
    orderedPolynomial("1+x+y+z+x^2+y^2+z^2+xy+xz+yz+x^3+y^3+z^3+x^2y+xy^2+x^2z+xz^2+y^2z+yz^2+xyz", monomialGRevLexOrder, "x^3 + x^2y + xy^2 + y^3 + x^2z + xyz + y^2z + xz^2 + yz^2 + z^3 + x^2 + xy + y^2 + xz + yz + z^2 + x + y + z + 1");
})
...
describe('polynomial division', () => {
    ...
    divisionRemainder("r_p^2 + (r_o^2 -x_o^2 + 2x_ox_p -x_p^2 -y_o^2)", ["a_a^2r_p^2 + r_p^2 -y_p^2 + (2a_ax_p + 2b_a)y_p"], "y_p^2 + (-2a_ax_p -2b_a)y_p + ({a_a}^{2}{r_o}^{2} -{a_a}^{2}{x_o}^{2} + 2{a_a}^{2}x_ox_p -{a_a}^{2}{x_p}^{2} -{a_a}^{2}{y_o}^{2} + {r_o}^{2} -{x_o}^{2} + 2x_ox_p -{x_p}^{2} -{y_o}^{2})", ["y_p", "r_p"])
});

describe('groebner basis', () => {
    grobner(["x^3-2xy", "x^2y-2y^2+x"], "grlex", ["x^2", "xy", "y^2-1/2x"]);
    grobner(["xz-y^2", "x^3-z^2"], "grlex", ["xz-y^2", "x^3-z^2", "x^2y^2-z^3", "xy^4-z^4, y^6-z^5"]);
    grobner(["x^2+y^2+z^2-1", "x^2+z^2-y", "x-z"], "lex", ["x-z", "-y+2z^2", "z^4+1/2z^2-1/4"]);
    grobner(["t^4-x", "t^3-y", "t^2-z"], "lex", ["-t^2+z", "ty-z^2", "tz-y", "x-z^2", "y^2-z^3"]);
    grobner(["x^2+y+z-1", "x+y^2+z-1", "x+y+z^2-1"], "lex", ["x+y+z^2-1", "y^2-y-z^2+z", "2yz^2+z^4-z^2", "z^6-4z^4+4z^3-z^2"]);
