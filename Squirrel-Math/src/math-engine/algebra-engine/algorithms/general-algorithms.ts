...
export function numericValue(e: Expression): Decimal | Expression {
    if (e instanceof Quotient)
        return simplify(
            new Quotient(
                numericValue(e.numerator),
                numericValue(e.denominator),
            ),
        );
    if (e instanceof Sum)
    ...
    if (e instanceof Power)
        return simplify(
            new Power(numericValue(e.base), numericValue(e.exponent)),
        );
    if (e instanceof Number) return new Decimal(e.numeric());
    return e;
}

//returns product: (number)*(extracted expression)
export function extractConstantFactor(e: Expression): Product {
    let coefficients = standardForm(e).addends.map(
        (a) => (a as Product).factors[0] as Number,
    );
    const denominators = coefficients
        .filter((c) => c instanceof Fraction)
        .map((c) => (c as Fraction).denominator.int);
    const commonDenominator = new Integer(numericLCM(...denominators));
    coefficients = coefficients.map(
        (c) => simplify(c.multiply(commonDenominator)) as Number,
    );
    let gcd = numericGCD(
        ...coefficients
            .filter((c) => c instanceof Integer)
            .map((c) => (c as Integer).int),
    );
    if (gcd instanceof Infty) gcd = BigInt(1);
    if (coefficients[0] && coefficients[0].isNegative()) gcd = -gcd;
    const constantFactor = simplify(
        new Fraction(gcd, commonDenominator.int),
    ) as Number;
    return Product.of(
        constantFactor,
        simplify(Product.of(constantFactor.inverse(), e)),
    );
}
...
export function extractMonomialFactor(e: Expression): Product {
    ...
    if (addends.length == 0) return Product.of(Integer.one, Integer.zero);
        ...
        .forEach((f) => {
            ...
            if (
                ...
                extractFactors.push(
                    minExponent == BigInt(1)
                        ? b
                        : new Power(b, new Integer(minExponent)),
                );
        });
...
export function standardForm(e: Expression): Sum {
    let resultAddends;
    if (e instanceof Sum) resultAddends = [...e.addends];
    else resultAddends = [e];
    for (let i = 0; i < resultAddends.length; i++) {
        let resultFactors;
        if (!(resultAddends[i] instanceof Product))
            resultFactors = [resultAddends[i]];
        else resultFactors = [...(resultAddends[i] as Product).factors];
