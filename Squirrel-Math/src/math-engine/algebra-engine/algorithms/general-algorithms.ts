...
import { simplify, orderProduct } from './simplification-algorithm';
...
export function extractConstantFactor(e: Expression): Product {
    ...
    let denominators = coefficients.filter(c => c instanceof Fraction).map(c => (c as Fraction).denominator.int);
    let commonDenominator = new Integer(numericLCM(...denominators));
    ...
    if (coefficients[0] && coefficients[0].isNegative())
        gcd = -gcd
    let constantFactor = simplify(new Fraction(gcd, commonDenominator.int)) as Number;
...
export function extractMonomialFactor(e: Expression): Product {
    let numberExtraction = extractConstantFactor(e);
    let extractFactors: Expression[] = [numberExtraction.factors[0]];

    let addends = standardForm(e).addends;
    if (addends.length == 0)
        return Product.of(Integer.one, Integer.zero);
    (addends[0] as Product).factors.filter((f, i) => i > 0).forEach(f => {
        ...
        if (addends.filter((a, i) => i > 0).every(a => (a as Product).factors.some(g => {
            if (g instanceof Power && g.base.identical(b) && g.exponent instanceof Integer && g.exponent.int <= minExponent)
    ...
    let extract = Product.of(...extractFactors);
