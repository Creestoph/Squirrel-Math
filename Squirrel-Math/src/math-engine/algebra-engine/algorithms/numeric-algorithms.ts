...
export function numericGCD(...values: bigint[]): bigint | Infty {
    let gcdFor2Args = function (a: bigint | Infty, b: bigint | Infty): bigint | Infty {
        if ((a instanceof Infty || a == BigInt(0)) && (b instanceof Infty || b == BigInt(0)))
            return Infty.positive;
        if (a instanceof Infty || a == BigInt(0))
            return b instanceof Infty ? Infty.positive : b < 0 ? -b : b;
        if (a < 0)
            a = -a;
        if (b instanceof Infty || b == BigInt(0))
            return a;
        if (b < 0)
            b = -b;
    ...
    if (values.length == 0)
        return Infty.positive;
    return values.reduce((gcd: bigint | Infty, v) => gcdFor2Args(gcd, v), BigInt(0));
}

export function numericLCM(...values: bigint[]): bigint {
    let lcmFor2Args = function (a: bigint, b: bigint): bigint {
        let gcd = numericGCD(a, b);
