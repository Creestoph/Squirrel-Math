...
export function isInequalityIdentity(e: Inequality): boolean {
    ...
    if (own.left instanceof Number && own.right instanceof Number)
        switch(own.sign) {
            case InequalitySign.LESS: return own.left.lessThan(own.right);
            case InequalitySign.LESS_EQUAL: return own.left.lessThan(own.right) || equals(own.left, own.right);
            case InequalitySign.GREATER: return !own.left.lessThan(own.right) && !equals(own.left, own.right);
            case InequalitySign.GREATER_EQUAL: return !own.left.lessThan(own.right);
        }
...
export function isEquationContradictory(e: Equation): boolean {
    ...
    return isInequalityIdentity(new Inequality(own.left, InequalitySign.GREATER, own.right)) || 
...
export function solvePolynomialEquation(u: UnivariatePolynomial): Set {
    ...
    if (u.degree() == 2) {
        let solutions = new QuadraticEquation(u.coefficients[2], u.coefficients[1], u.coefficients[0], u.variable).solve().map(s => simplify(s));
    ...
    let numericSolutions = factorized.filter(f => f.degree() > 2).map(f => solveNumeric(new Equation(f, Integer.zero), f.variable))
...
export function solveLinearSystem(matrix: Expression[][], values: Expression[]): Expression[] {
	...
	for (let column = 0; column < n; column++) {
		...
		for (let j = column + 1; j < n; j++) {
			if (!equals(matrix[column][j], Integer.zero) && equals((matrix[column][biggest_in_column]), Integer.zero) ||
                matrix[column][j] instanceof Number && !equals(matrix[column][j], Integer.zero) && !(matrix[column][biggest_in_column] instanceof Number) || 
                matrix[column][j] instanceof Number && matrix[column][biggest_in_column] instanceof Number && (matrix[column][biggest_in_column] as Number).lessThan(matrix[column][j] as Number))
                biggest_in_column = j;
        }
		...
		for (let eliminated_row_index = column + 1; eliminated_row_index < n; eliminated_row_index++) {
			...
			for (let i = column + 1; i < n; i++)
                matrix[i][eliminated_row_index] = simplify(Sum.difference(Product.of(matrix[i][eliminated_row_index], c1), Product.of(matrix[i][column], c2)));
			values[eliminated_row_index] = simplify(Sum.difference(Product.of(values[eliminated_row_index], c1), Product.of(values[column], c2)));
		}
	}
	...
	for (let row_index = n - 1; row_index >= 0; row_index--) {
		for (let i = row_index + 1; i < n; i++)
            values[row_index] = simplify(Sum.difference(values[row_index], Product.of(matrix[i][row_index], result[i])));
...
export function solvePolynomialSystem(polys: Polynomial[], variables: Variable[]): Expression[][] {
    ...
    let isLinear = true;
    polys.forEach((p, i) => p.monomials.forEach(m => {
    ...
    if (isLinear)
        return [solveLinearSystem(coefficientAddents.map(c => c.map(x => simplify(Sum.of(...x)))), freeAddends.map(f => simplify(Sum.of(...f))))];
    ...
    let solveGrobnerSystem = function(grobnerPolys: Polynomial[], vars: Variable[]) {
        for (let g = 0; g < grobnerPolys.length; g++) {
            ...
            if (variablesInG.length == 1) {
                ...
                let solutions = solvePolynomialEquation(UnivariatePolynomial.fromPolynomial(grobnerPolys[g], variablesInG[0]));
                ...
                if (solutions instanceof FiniteSet) {
                    ...
                    if (remainingVariables.length == 0) {
                        ...
                        solutions.elements.forEach(e => {
                            let substituted = grobnerPolys.map(p => Polynomial.asPolynomial(p.substitute(variablesInG[0], e)));
                            let recursiveSolutions = solveGrobnerSystem(substituted, remainingVariables);
                            allSolutions.push(...recursiveSolutions.map(r => {
...
export function solveNumeric(e: Equation, x: Variable, lowerBound = -100, upperBound = 100, step = 0.01, precision = 0.001): FiniteSet {
    ...
    for (let pos = lowerBound; pos < upperBound; pos += step) {
        ...
        if (!(v instanceof Number))
            throw new Error("Can't solve equation " + e.toMathJax() + " using numeric methods; it containt more than one variable");
        let sign = v.signum();
        if (sign == 0)
    ...
    if (UnivariatePolynomial.isPolynomial(e.left, x)) {
        ...
        if (coefficients.length == 2) {
            ...
            if (isInequalityIdentity(new Inequality(coefficients[1], InequalitySign.GREATER, Integer.zero))) {
                switch(e.sign) {
                    case InequalitySign.GREATER: return new Interval(zeroPoint, Infty.positive, true, true);
                    case InequalitySign.GREATER_EQUAL: return new Interval(zeroPoint, Infty.positive, false, true);
                    case InequalitySign.LESS: return new Interval(Infty.negative, zeroPoint, true, true);
                    case InequalitySign.LESS_EQUAL: return new Interval(Infty.negative, zeroPoint, true, false);
                }
            }
            else if (isInequalityIdentity(new Inequality(coefficients[1], InequalitySign.LESS, Integer.zero))) {
                switch(e.sign) {
                    case InequalitySign.GREATER: return new Interval(Infty.negative, zeroPoint, true, true);
                    case InequalitySign.GREATER_EQUAL: return new Interval(Infty.negative, zeroPoint, true, false);
                    case InequalitySign.LESS: return new Interval(zeroPoint, Infty.positive, true, true);
                    case InequalitySign.LESS_EQUAL: return new Interval(zeroPoint, Infty.positive, false, true);
                }
            }
            else 
                throw new Error("Can't solve inequality " + e.toMathJax() + " in general case as coefficient " + 
                coefficients[1].toMathJax() + " has undefined sign");
        }

        if (coefficients.length == 3) {
            let solutions = new QuadraticEquation(coefficients[2], coefficients[1], coefficients[0], x).solve().map(s => simplify(s));
            ...
            if (solutions.length == 0)
                switch(e.sign) {
                    ...
                    case InequalitySign.GREATER_EQUAL: return Interval.realLine();
                    case InequalitySign.LESS: 
                    case InequalitySign.LESS_EQUAL: return Set.empty;
                }
            else if (solutions.length == 1)
                switch(e.sign) {
                    case InequalitySign.GREATER: return new SetSum(new Interval(Infty.negative, solutions[0], true, true), 
                        new Interval(solutions[0], Infty.positive, true, true));
                    case InequalitySign.GREATER_EQUAL: return Interval.realLine();
                    case InequalitySign.LESS: return Set.empty;
                    case InequalitySign.LESS_EQUAL: return new FiniteSet(solutions[0]);
                }
            else {
                let lesser, greater;
                if (isInequalityIdentity(new Inequality(solutions[0], InequalitySign.LESS, solutions[1]))) {
                    ...
                    throw new Error("Can't solve inequality " + e.toMathJax() + " as it has two incomparable zeros: " + 
                    solutions[0].toMathJax() + " and " + solutions[1].toMathJax());

                switch(e.sign) {
                    case InequalitySign.GREATER: return new SetSum(new Interval(Infty.negative, lesser, true, true), 
                        new Interval(greater, Infty.positive, true, true));
                    case InequalitySign.GREATER_EQUAL: return new SetSum(new Interval(Infty.negative, lesser, true, false), 
                    new Interval(greater, Infty.positive, false, true));
                    case InequalitySign.LESS: return new Interval(lesser, greater, true, true);
                    case InequalitySign.LESS_EQUAL: return new Interval(lesser, greater, false, false);
                }
            }
        }
    }
