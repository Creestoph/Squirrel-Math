...
export function isInequalityIdentity(e: Inequality): boolean {
    ...
    if (own.left instanceof Number && own.right instanceof Number)
        switch (own.sign) {
            ...
            case InequalitySign.LESS_EQUAL:
                return (
                    own.left.lessThan(own.right) || equals(own.left, own.right)
                );
            case InequalitySign.GREATER:
                return (
                    !own.left.lessThan(own.right) &&
                    !equals(own.left, own.right)
                );
    ...
    try {
        return poly.variables.every((v) =>
            solveInequality(e, v).equals(Interval.realLine()),
        );
    } catch (e) {}
    return false;
}
...
export function isEquationContradictory(e: Equation): boolean {
    ...
    return (
        isInequalityIdentity(
            new Inequality(own.left, InequalitySign.GREATER, own.right),
        ) ||
        isInequalityIdentity(
            new Inequality(own.left, InequalitySign.LESS, own.right),
        )
    );
}
...
export function solvePolynomialEquation(u: UnivariatePolynomial): Set {
    ...
    if (u.degree() == 0)
        return equals(u.coefficients[0], Integer.zero)
            ? Interval.realLine()
            : Set.empty;
    if (u.degree() == 1)
    ...
    if (u.degree() == 2) {
        let solutions = new QuadraticEquation(
            u.coefficients[2],
            u.coefficients[1],
            u.coefficients[0],
            u.variable,
    ...
    let factorized = factorizePolynomial(u).factors.map((f) =>
        UnivariatePolynomial.fromPolynomial(
            Polynomial.asPolynomial(f),
            u.variable,
        ),
    );
    let preciseSolutions = factorized
        .filter((f) => f.degree() <= 2)
        .map((f) => solvePolynomialEquation(f));
...
export function solveEquation(e: Equation, x: Variable): Set {
    ...
    if (UnivariatePolynomial.isPolynomial(e.left, x))
        return solvePolynomialEquation(
            UnivariatePolynomial.fromPolynomial(
                Polynomial.asPolynomial(e.left),
                x,
            ),
        );
...
export function solveLinearSystem(
    ...
    for (let column = 0; column < n; column++) {
        ...
        for (let j = column + 1; j < n; j++) {
            if (
                (!equals(matrix[column][j], Integer.zero) &&
                    equals(matrix[column][biggest_in_column], Integer.zero)) ||
                (matrix[column][j] instanceof Number &&
                    !equals(matrix[column][j], Integer.zero) &&
                    !(matrix[column][biggest_in_column] instanceof Number)) ||
                (matrix[column][j] instanceof Number &&
                    matrix[column][biggest_in_column] instanceof Number &&
                    (matrix[column][biggest_in_column] as Number).lessThan(
                        matrix[column][j] as Number,
                    ))
            )
                biggest_in_column = j;
        }
        ...
        for (
            let eliminated_row_index = column + 1;
            eliminated_row_index < n;
            eliminated_row_index++
        ) {
            ...
            for (let i = column + 1; i < n; i++)
                matrix[i][eliminated_row_index] = simplify(
                    Sum.difference(
                        Product.of(matrix[i][eliminated_row_index], c1),
                        Product.of(matrix[i][column], c2),
                    ),
                );
            values[eliminated_row_index] = simplify(
                Sum.difference(
                    Product.of(values[eliminated_row_index], c1),
                    Product.of(values[column], c2),
                ),
            );
        }
    }
    ...
    for (let row_index = n - 1; row_index >= 0; row_index--) {
        for (let i = row_index + 1; i < n; i++)
            values[row_index] = simplify(
                Sum.difference(
                    values[row_index],
                    Product.of(matrix[i][row_index], result[i]),
                ),
            );
        if (equals(matrix[row_index][row_index], Integer.zero))
            throw new Error('Contradictory system');
        result[row_index] = simplify(
            new Quotient(values[row_index], matrix[row_index][row_index]),
        );
    }
...
export function solvePolynomialSystem(
    ...
    let coefficientAddents: Expression[][][] = variables.map((x) =>
        polys.map((p) => []),
    );
    ...
    polys.forEach((p, i) =>
        p.monomials.forEach((m) => {
            if (
                m.variables.filter((x) => variables.some((v) => v.identical(x)))
                    .length > 1
            )
                isLinear = false;
            let k = -1;
            ...
            for (let j = 1; j < m.factors.length; j++) {
                k = variables.findIndex((x) =>
                    x.identical((m.factors[j] as Power).base),
                );
                if (k != -1) {
    ...
    if (isLinear)
        return [
            solveLinearSystem(
                coefficientAddents.map((c) =>
                    c.map((x) => simplify(Sum.of(...x))),
                ),
    ...
    if (
        grobner.some((p) =>
            isEquationContradictory(new Equation(p, Integer.zero)),
        )
    ) {
        return [];
    }
    let solveGrobnerSystem = function (
        grobnerPolys: Polynomial[],
        vars: Variable[],
    ) {
        for (let g = 0; g < grobnerPolys.length; g++) {
            let variablesInG = vars.filter((x) =>
                grobnerPolys[g].variables.some((v) => v.identical(x)),
            );
                        ...
                        solutions.elements.forEach((e) => {
                            let substituted = grobnerPolys.map((p) =>
                                Polynomial.asPolynomial(
                                    p.substitute(variablesInG[0], e),
                                ),
                            );
                            let recursiveSolutions = solveGrobnerSystem(
                                substituted,
                                remainingVariables,
                            );
                            allSolutions.push(
...
export function solveNumeric(
    ...
    let bisectionInRange = function (l: number, u: number) {
        ...
        while (Math.abs(u - l) > precision) {
            let middle = (l + u) / 2;
            let s = (
                simplify(f.substitute(x, new Decimal(middle))) as Number
            ).signum();
            if (s == 0) break;
        ...
        if (!(v instanceof Number))
            throw new Error(
                "Can't solve equation " +
                    e.toMathJax() +
                    ' using numeric methods; it containt more than one variable',
            );
        let sign = v.signum();
...
export function solveInequality(e: Inequality, x: Variable): Set {
    e.simplified();
    if (UnivariatePolynomial.isPolynomial(e.left, x)) {
        let coefficients = UnivariatePolynomial.fromPolynomial(
            Polynomial.asPolynomial(e.left),
            x,
        ).coefficients;
        if (coefficients.length == 2) {
            let zeroPoint = simplify(
                new LinearEquation(coefficients[1], coefficients[0], x).solve(),
            );
            if (
                isInequalityIdentity(
                    new Inequality(
        ...
        if (coefficients.length == 3) {
            ...
            if (
                ...
                coefficients = coefficients.map(
                    (c, i) =>
                        (coefficients[i] = Sum.of(
                            Product.of(Integer.minusOne, c),
                        )),
                );
