...
export class Polynomial extends Sum {
    ...
    protected constructor(monomials: Monomial[], order: Order) {
        ...
        this.monomials.forEach((m) =>
            m.variables.forEach((v) => (varByName[v.toMathJax()] = v)),
        );
    ...
    static fromMonomials(
        monomials: Monomial[],
        order = monomialLexOrder,
    ): Polynomial {
    ...
    static asPolynomial(
        ...
        if (
            e instanceof Polynomial &&
            (!variables ||
                (e.variables.length == variables.length &&
                    e.variables.every((v) =>
                        variables.some((w) => v.identical(w)),
                    ))) &&
            e.order == order
        )
            return e;
        return Polynomial.fromMonomials(
            standardForm(e).addends.map(
                (p) => Monomial.asMonomial(p, variables) as Monomial,
            ),
            order,
        ).cleaned();
    }
    ...
    substitute(old: Expression, e: Expression): Expression {
        const result = super.substitute(old, e);
        if (result instanceof Sum) return Sum.of(...result.addends);
        return result;
    }
    ...
    cleaned(): Polynomial {
        ...
        for (let i = 0; i < monos.length - 1; i++)
            if (
                monos[i].variables.length == monos[i + 1].variables.length &&
                monos[i].variables.every((v, j) =>
                    v.identical(monos[i + 1].variables[j]),
                ) &&
                monos[i].exponentVector.every((v, j) =>
                    v.identical(monos[i + 1].exponentVector[j]),
                )
            ) {
                monos[i] = new Monomial(
                    Sum.joined(monos[i].coefficient, monos[i + 1].coefficient),
                    [...monos[i].variables],
        ...
        monos = monos
            .map(
                (m) =>
                    new Monomial(
                        simplify(m.coefficient),
                        [...m.variables],
                        [...m.exponentVector],
                    ),
            )
            .filter((m) => !m.coefficient.identical(Integer.zero));
    ...
    add(addend: Monomial | Polynomial): Polynomial {
        if (addend instanceof Monomial)
            return Polynomial.fromOrderedMonomials(
                insertByBisection(
                    [...this.monomials],
                    addend,
        ...
        let lastInserted = 0;
        monos2.forEach((m) => {
            lastInserted = findByBisection(
                monos1,
                m,
                dominatingOrder.descending,
                lastInserted,
            );
            if (
                monos1[lastInserted] &&
                monos1[lastInserted].body().identical(m.body())
            ) {
                const newCoefficient = simplify(
                    Sum.of(monos1[lastInserted].coefficient, m.coefficient),
                );
                ...
                monos1[lastInserted - 1] &&
                monos1[lastInserted - 1].body().identical(m.body())
            ) {
                const newCoefficient = simplify(
                    Sum.of(monos1[lastInserted - 1].coefficient, m.coefficient),
                );
                if (newCoefficient.identical(Integer.zero))
                    monos1.splice(--lastInserted, 1);
                else
            ...
            } else monos1.splice(lastInserted, 0, m);
        });
    ...
    multiply(factor: Monomial | Polynomial): Polynomial {
        if (factor instanceof Monomial)
            return Polynomial.fromOrderedMonomials(
                this.monomials.map((m) => m.multiply(factor)),
                this.order,
            );
        return Polynomial.fromMonomials(
            factor.monomials.flatMap((m) => this.multiply(m).monomials),
            this.order,
        ).cleaned();
    }
}
