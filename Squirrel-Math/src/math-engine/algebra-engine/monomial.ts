...
export class Monomial extends Product {
    ...
    constructor(
        ...
        super([
            coefficient,
            ...variables.map((v, i) => new Power(v, exponentVector[i])),
        ]);
    ...
    static asMonomial(e: Expression, variables?: Variable[]): Monomial {
        if (e instanceof Monomial) return e;
        const s = standardForm(e);
        if (s.addends.length > 1) return new Monomial(e, [], []);
        const coefficientFactors = [];
        const vars = [];
        const exponentVector = [];

        for (let i = 0; i < (s.addends[0] as Product).factors.length; i++) {
            const p = (s.addends[0] as Product).factors[i] as Power;
            if (
                p.base instanceof Variable &&
                (!variables || variables.some((v) => v.identical(p.base))) &&
                p.exponent instanceof Integer &&
                !p.exponent.isNegative()
            ) {
                vars.push(p.base);
                exponentVector.push(p.exponent);
            } else coefficientFactors.push(p);
        }
        const coefficient =
            coefficientFactors.length == 1
                ? coefficientFactors[0]
                : Product.of(...coefficientFactors);
        return new Monomial(coefficient, vars, exponentVector);
    }

    copy(): Monomial {
        return new Monomial(
            this.coefficient.copy(),
            this.variables.map((v) => v.copy()),
            this.exponentVector.map((e) => e.copy()),
        );
    }

    substitute(old: Expression, e: Expression): Expression {
        const result = super.substitute(old, e);
        if (result instanceof Product) return Product.of(...result.factors);
        return result;
    }
    ...
    toMathJax(): string {
        let result = '';
        if (
            this.coefficient.identical(Integer.minusOne) &&
            this.variables.length > 0
        )
            result += '-';
        else if (
            !(
                this.coefficient.identical(Integer.one) &&
                this.variables.length > 0
            )
        )
            result +=
        ...
        this.exponentVector
            ...
            .forEach((f, i) => {
                ...
                if (!f.identical(Integer.one)) result += '^' + f.toMathJax();
            });
        return result;
    }
    ...
    cleaned(): Monomial {
        ...
        for (let i = 0; i < factors.length - 1; i++)
            if (factors[i].base.identical(factors[i + 1].base)) {
                factors[i] = new Power(
                    factors[i].base,
                    (factors[i].exponent as Integer).add(
                        factors[i + 1].exponent as Integer,
                    ),
                );
        ...
        return new Monomial(
            this.coefficient,
            factors.map((f) => (f as Power).base as Variable),
    ...
    multiply(m: Monomial | Expression): Monomial {
        if (m instanceof Monomial)
            return new Monomial(
                simplify(Product.of(this.coefficient, m.coefficient)),
                this.variables.concat(m.variables),
                this.exponentVector.concat(m.exponentVector),
            ).cleaned();
        else
            return new Monomial(
                simplify(Product.of(this.coefficient, m)),
                [...this.variables],
                [...this.exponentVector],
            );
    }
