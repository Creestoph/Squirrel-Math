...
export function parseExpression(text: string): Expression {
            ...
            while (open > 0) {
                if (position == text.length)
                    throw (
                        ...
                        (openPosition == 0
                            ? 'at the beginning'
                            : 'after ' + text.substr(0, openPosition - 1)) +
                        ' in expression ' +
                        text
                    );
                if (text[position] == char) open++;
                if (text[position] == bracketPairs[char]) open--;
                if (open > 0) bracket += text[position];
                position++;
            }
    ...
    const injectToken = function () {
        if (lastToken) {
            if (rememberedMinus) {
                ...
                else lastToken = Product.of(Integer.minusOne, lastToken);
                rememberedMinus = false;
            }
            if (!addend) addend = lastToken;
            else if (addend instanceof Product)
                addend = Product.of(...addend.factors, lastToken);
            else addend = Product.of(addend, lastToken);
        }
    };
    ...
    while (position < text.length) {
        ...
        if (char == '+' || char == '-') {
            ...
            injectToken();
            if (addend) resultAddends.push(addend);
            lastToken = null;
            addend = null;
            position++;
            if (char == '-') rememberedMinus = !rememberedMinus;
        } else if (char == '*') {
            if (!lastToken)
                throw (
                    'Wrong character * ' +
            ...
            else lastToken = new Variable(lastToken.name, index);
        } else if (char == '^') {
            ...
            else lastToken = new Quotient(lastToken, denominator);
        } else {
            injectToken();
            lastToken = nextToken();
            expectToken = false;
    ...
    injectToken();
    if (addend) resultAddends.push(addend);
    if (resultAddends.length == 0) return Integer.zero;
    if (resultAddends.length == 1) return resultAddends[0];
