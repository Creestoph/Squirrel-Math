...
export class Point implements Figure {
    ...
    translated(v: Vector): Point {
        return new Point(simplify(Sum.of(this.x, v.x)), simplify(Sum.of(this.y, v.y)), this.name);
    }
    scaled(scale: Expression, p: Point = new Point(Integer.zero, Integer.zero)): Point {
        return new Point(simplify(Sum.of(p.x, Product.of(Sum.difference(this.x, p.x), scale))), 
        simplify(Sum.of(p.y, Product.of(Sum.difference(this.y, p.y), scale))), this.name);
    }

    distanceTo(p: Point): Power {
        return new Power(Sum.of(new Power(Sum.difference(p.x, this.x), new Integer(2)), 
        new Power(Sum.difference(p.y, this.y), new Integer(2))), new Fraction(1, 2));
    }
}