...
export class Circle implements Figure {
    ...
    draw() {
        if (this.center.x instanceof Number && this.center.y instanceof Number && this.r instanceof Number) {
            let circle = new paper.Path.Circle(new paper.Point(this.center.x.numeric(), this.center.y.numeric()), 
                this.r.numeric());
    ...
    scaled(scale: Expression, p: Point = new Point(Integer.zero, Integer.zero)): Circle {
        return new Circle(this.center.scaled(scale, p), simplify(Product.of(this.r, scale)));
    }

    equation(paramX: Expression = new Variable("x"), paramY: Expression = new Variable("y")): Equation {
        return new Equation((this.center.distanceTo(new Point(paramX, paramY)) as Power).base, new Power(this.r, new Integer(2)));
    }
}