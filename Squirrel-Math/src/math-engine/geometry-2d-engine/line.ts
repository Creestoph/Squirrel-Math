...
import {
    simplify,
    equals,
} from '../algebra-engine/algorithms/simplification-algorithm';
...
export class Line implements Figure {
    ...
    static vertical(x: Expression, name?: string): Line {
        return new Line(
            Integer.one,
            Integer.zero,
            simplify(Product.of(Integer.minusOne, x)),
            name,
        );
    }
    ...
    static hotizontal(y: Expression, name?: string): Line {
        return new Line(
            Integer.zero,
            Integer.one,
            simplify(Product.of(Integer.minusOne, y)),
            name,
        );
    }
    ...
    static fromPoints(p1: Point, p2: Point, name?: string): Line {
        return new Line(
            Sum.difference(p2.y, p1.y),
            Sum.difference(p1.x, p2.x),
            Sum.difference(Product.of(p2.x, p1.y), Product.of(p1.x, p2.y)),
            name,
        );
    }
    ...
    draw() {
        if (
            this.a instanceof Number &&
            this.b instanceof Number &&
            this.c instanceof Number
        ) {
            let line;
            if (!this.isVertical()) {
                let x1 = -1000;
                let y1 = (
                    simplify(
                        this.directionalEquation(new Integer(-1000)).right,
                    ) as Number
                ).numeric();
                let x2 = 1000;
                let y2 = (
    ...
    translated(v: Vector): Line {
        return new Line(
            ...
            simplify(
                Sum.of(
                    this.c,
                    Product.of(Integer.minusOne, this.a, v.x),
                    Product.of(Integer.minusOne, this.b, v.y),
                ),
            ),
            this.name,
        );
    ...
    scaled(
        ...
        result = new Line(
            result.a,
            result.b,
            simplify(Product.of(this.c!, scale)),
            this.name,
        );
    ...
    directionalEquation(
        ...
        if (this.isVertical())
            throw (
                'Line ' +
                this.name +
                ' has no directional equation as it is vertical'
            );
        let aDirectional = Product.of(
            Integer.minusOne,
            new Quotient(this.a, this.b),
        );
        let bDirectional = Product.of(
            Integer.minusOne,
            new Quotient(this.c, this.b),
        );
        return new Equation(
            paramY,
            Sum.of(Product.of(aDirectional, paramX), bDirectional),
        );
    }
    ...
    generalEquation(
        ...
        return new Equation(
            Sum.of(
                Product.of(this.a, paramX),
                Product.of(this.b, paramY),
                this.c,
            ),
            Integer.zero,
        );
