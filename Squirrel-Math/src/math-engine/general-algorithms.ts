...
export function findByBisection(
    ...
    let leftBound = from;
    let rightBound = to;
    if (order(element, array[rightBound]) > 0) return rightBound + 1;
    if (order(element, array[leftBound]) < 0) return leftBound;
    let middle = Math.floor((leftBound + rightBound) / 2);
    while (rightBound - leftBound > 1) {
        const o = order(element, array[middle]);
        if (o == 0) return middle;
        else if (o < 0) rightBound = middle;
        else if (o > 0) leftBound = middle;
...
export function insertByBisection(
    array: any[],
    element: any,
    order: (a: any, b: any) => number,
): any[] {
