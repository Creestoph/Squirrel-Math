...
import { Number, Infty } from '../algebra-engine/numbers';
    ...
    randomElement(): Expression | undefined {
        ...
        if (own.addends.some((a) => !a.isFinite()))
            for (let i = 0; i < own.addends.length; i++)
                if (own.addends[i].isFinite()) own.addends.splice(i--, 1);
        ...
        own.addends.forEach((a) => {
            let aSize = a.size();
            totalSize +=
                aSize instanceof Number
                    ? equals(aSize, Infty.positive)
                        ? 1
                        : aSize.numeric()
                    : 1;
        });
